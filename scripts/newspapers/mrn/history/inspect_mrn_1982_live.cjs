...
async function main(){
const dates=fs.readdirSync('C:/Users/schaf/Desktop/MRN/1982',{withFileTypes:true}).filter(x=>x.isDirectory()).map(x=>{let [m,d,y]=x.name.split('-');return `19${y}-${m.padStart(2,'0')}-${d.padStart(2,'0')}`;}).sort();
const remote=(await api('/storage/v1/object/list/media',{prefix:'newspapers/midwest-racing-news',search:'1982-',limit:1000})).map(x=>x.name).filter(x=>/^1982-\d\d-\d\d$/.test(x)).sort();
