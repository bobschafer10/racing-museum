...
source = Path('C:/Users/schaf/Desktop/MRN/1982')
root = Path('C:/Users/schaf/Desktop/PHOTOS II/mrn-prepared-1982')
if root.exists():
...
for folder in source.iterdir():
    ...
    assert issue_date.startswith('1982-')
...
for issue_date, folder, files in sorted(plan):
    ...
    issues.append(dict(issue_date=issue_date,source_folder=str(folder),source_type='existing_jpeg_pages_no_pdf',page_count=len(files),output_path=str(destdir),date_evidence='source folder '+folder.name,date_uncertain=False,front_page_verified=issue_date=='1982-04-02',quality_review_pages=';'.join(warnings)))
for name,rows in [('issues.csv',issues),('pages-upload-manifest.csv',pages)]:
    with (root/name).open('w',newline='',encoding='utf-8') as f:
        w=csv.DictWriter(f,fieldnames=list(rows[0])); w.writeheader(); w.writerows(rows)
(root/'audit.json').write_text(json.dumps(dict(source=str(source),processed_year=1982,stopped_for_verification=True,originals_unchanged=True,copy_hashes_verified=True,upload_performed=False,issues=issues,pages=pages),indent=2),encoding='utf-8')
(root/'README.txt').write_text('1982 ONLY. Stopped for user verification. Originals copied byte-for-byte, not modified. Source contains JPG pages, no PDFs. Dates parsed as month-day-year-issue from source folders; only April 2 front page visually verified. Review quality warnings before uploading. Upload manifest targets existing media bucket with image/jpeg and upsert=false. No uploads performed. Do not replace any existing object. 1959-1980 and 1982 onward untouched.\n',encoding='utf-8')
