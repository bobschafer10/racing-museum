...
const lastRecordedYear =
  safeResultsByYear.length > 0
    ? safeResultsByYear[0]?.result_year
    : null
...
<div style={heroDivider}>
  ...
  <HeroStat
    ...
    value={driver.recorded_wins ?? 0}
  />

  <HeroStat
    label="Wisconsin Feature Wins"
    value={driver.wisconsin_feature_wins ?? 0}
  />

  <HeroStat
    label="Recorded Results"
    value={driver.recorded_results ?? 0}
  />

<HeroStat
  label="First Recorded Year"
  value={firstRecordedYear ?? 0}
  format={false}
/>

<HeroStat
  label="Last Recorded Year"
  value={lastRecordedYear ?? 0}
  format={false}
/>
</div>
  ...
  <div style={mainGrid}>
    <div style={leftColumn}>
      <Panel title="Driver Summary">
        ...
        <SummaryRow
          label="Hometown"
          value={driver.hometown || 'Unknown hometown'}
        />
          ...
          <div>
            {safeRecentResults.map((result, index) => (
              ...
              >
                ...
                <span style={recentClass}>
                  {result.class_name || 'Unknown'}
                </span>

                <span style={recentPosition}>
    ...
    <div style={rightColumn}>
      <Panel title="Quick Stats">
        <SummaryRow
          label="Recorded Feature Wins"
          value={String(driver.recorded_wins ?? 0)}
        />
        <SummaryRow
          label="Wisconsin Feature Wins"
          value={String(driver.wisconsin_feature_wins ?? 0)}
        />
        <SummaryRow
          label="Top 3 Feature Finishes"
          value={String(driver.recorded_top_3_finishes ?? 0)}
        />
        <SummaryRow
          label="Total Feature Race Results"
          value={String(driver.recorded_results ?? 0)}
        />
      </Panel>
...
<Panel title="Track Championships">
  ...
  <div style={champDetails}>
    ...
    {ch.class_name && (
      <div style={champClass}>{ch.class_name}</div>
    )}
  </div>
</div>
      ))}
...
function CareerHighlights({
  ...
  highlights: { year: number | string; text: string }[]
}) {
  ...
  return (
    <div style={careerHighlightsBox}>
      <div style={careerHighlightsTitle}>
        <span style={careerHighlightsStar}>★</span>
        Career Highlights
      </div>
...
function buildPhotoCaption(photo: Photo) {
  const trackLabel =
    formatTrackSlug(photo.track_slug) || null
...
const creditType =
  ...
  photographer !== 'Unknown Credit'
    ? `${photographer}${creditType !== 'Photo' ? ` ${creditType}` : ''}`
    : null,
].filter(Boolean)
...
function parsePhotoFileName(fileName: string | null | undefined) {
  if (!fileName) {
    return {
      year: null,
  ...
  const yearPart = parts[1] || null
  const photographerPart = parts[3] || null
  const creditTypePart = parts[4] || null

  return {
    year:
      !yearPart || yearPart === 'unknown-year'
        ? null
        : yearPart.replace(/-/g, ' '),
    photographer:
      !photographerPart || photographerPart === 'unknown-credit'
        ? 'Unknown'
        : formatName(photographerPart),
    creditType:
      !creditTypePart || creditTypePart === 'unknown'
        ? 'Photo'
        : formatCreditType(creditTypePart),
  }
}
...
const heroWatermark: CSSProperties = {
  ...
  inset: 0,
  background:
    `linear-gradient(to right, rgba(234,223,199,0.25), rgba(234,223,199,0.72)),
     radial-gradient(circle at 78% 28%, rgba(80,55,25,0.18), transparent 34%),
     repeating-conic-gradient(
       from 45deg at 78% 25%,
       rgba(80,55,25,0.10) 0deg 10deg,
...
const photoPanel: CSSProperties = {
  border: '2px solid #bda87a',
  padding: '10px',
  background: '#f4ead7',
  boxShadow: '0 8px 24px rgba(0,0,0,0.12)',
}

const heroMainPanel: CSSProperties = {
  display: 'flex',
  flexDirection: 'column',
  gap: '12px',
}

const champRow: CSSProperties = {
  display: 'flex',
  gap: '12px',
  padding: '10px 0',
...
const champYear: CSSProperties = {
  width: '55px',
  fontWeight: 700,
  color: '#3d2b16',
}

const champDetails: CSSProperties = {
  display: 'flex',
  flexDirection: 'column',
}

const champTrack: CSSProperties = {
  fontSize: '14px',
  fontWeight: 700,
  color: '#2f2417',
  textDecoration: 'none',
...
const champClass: CSSProperties = {
  fontSize: '12px',
  color: '#7a6a55',
  marginTop: '2px',
}

const heroYearRow: CSSProperties = {
  display: 'grid',
...
const heroYearCard: CSSProperties = {
  background: '#f1e5ce',
...
const heroYearValue: CSSProperties = {
  fontSize: '28px',
  fontWeight: 700,
  color: '#2f2417',
  lineHeight: 1,
...
const heroYearLabel: CSSProperties = {
  fontSize: '13px',
  color: '#5a3a1b',
  lineHeight: 1.3,
}
...
const careerHighlightsBox: CSSProperties = {
...
const careerHighlightsTitle: CSSProperties = {
  display: 'flex',
  alignItems: 'center',
  gap: '8px',
  marginBottom: '10px',
...
const careerHighlightsStar: CSSProperties = {
  color: '#8a632b',
  fontSize: '15px',
}

const careerHighlightRow: CSSProperties = {
  display: 'grid',
...
const careerHighlightYear: CSSProperties = {
  fontWeight: 700,
  color: '#3d2b16',
}

const careerHighlightText: CSSProperties = {
  color: '#3f2d18',
}
...
const heroActionWrap: CSSProperties = {
  display: 'flex',
  alignItems: 'flex-start',
  paddingTop: '8px',
}
...
const heroStatCard: CSSProperties = {
  ...
  textAlign: 'center',
  color: '#fff7e7',
  borderRight: '1px solid rgba(255, 247, 231, 0.35)',
}
...
const heroDividerLine: CSSProperties = {
  height: '1px',
  flex: 1,
  background: '#b29364',
}

const heroDividerStar: CSSProperties = {
  color: '#9a743d',
  fontSize: '18px',
}
...
const recentTrackWrap: CSSProperties = {
  display: 'flex',
  alignItems: 'center',
  gap: '8px',
}

const recentTrackLogo: CSSProperties = {
  width: '24px',
  height: '24px',
  objectFit: 'contain',
}
...
const heroThumbRow: CSSProperties = {
  display: 'grid',
  gridTemplateColumns: 'repeat(3, 1fr)',
  gap: '8px',
  marginTop: '10px',
...
const recentRow: CSSProperties = {
  display: 'grid',
  gridTemplateColumns: '140px 1fr 140px 50px',
  alignItems: 'center',
  gap: '12px',
  padding: '8px 0',
  borderBottom: '2px solid #b29364',
...
const recentClass: CSSProperties = {
  fontSize: '14px',
  color: '#6b4a22',
  whiteSpace: 'nowrap',
}

const recentDate: CSSProperties = {
  fontSize: '14px',
  color: '#7a5827',
}

const recentTrack: CSSProperties = {
  color: '#5a3a1b',
  textDecoration: 'none',
  fontWeight: 500,
}

const recentPosition: CSSProperties = {
  fontWeight: 700,
  textAlign: 'right',
}
