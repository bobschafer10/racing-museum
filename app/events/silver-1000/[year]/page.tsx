...
export default async function Silver1000YearPage({ params }: { params: Promise<{ year: string }> }) {
  ...
  const [{ data: season }, { data: racesRaw, error }] = await Promise.all([
    supabase.from('SeriesSeasons').select('year,races').eq('series_id', SERIES_ID).eq('year', seasonYear).maybeSingle(),
    supabase.from('SeriesEvents').select(`id,race_date,winner_name,source_url,SeriesEventResults(id,finishing_position,starting_position,car_number,driver_name,status,result_section)`).eq('series_id', SERIES_ID).gte('race_date', `${seasonYear}-01-01`).lte('race_date', `${seasonYear}-12-31`).order('race_date', { ascending: true }),
  ])
    ...
    <main style={pageStyle}>
      <section style={hero}>
        <div style={heroInner}>
          ...
          <p style={tagline}>{cancelled ? 'Event cancelled.' : 'Late Model Division at Proctor Speedway.'}</p>
        </div>
      </section>
      ...
      <section style={contentWrap}>
        ...
        {cancelled ? (
          ...
          <div style={panel}><div style={panelBody}>Unable to load this year from the Museum database.</div></div>
        ) : races.length ? (
          races.map((race) => {
            const feature = race.SeriesEventResults.filter(r => r.result_section !== 'DNQ').sort((a,b) => (a.finishing_position ?? 9999) - (b.finishing_position ?? 9999))
            const dnq = race.SeriesEventResults.filter(r => r.result_section === 'DNQ').sort((a,b) => a.driver_name.localeCompare(b.driver_name))
            return (
              <div key={race.id} style={panel}>
                ...
                <div style={panelBody}>
                  ...
                  {dnq.length ? <div style={dnqWrap}><div style={dnqTitle}>Did Not Qualify</div>{dnq.map(row => <span key={row.id} style={dnqChip}>{row.driver_name}{row.car_number ? ` #${row.car_number}` : ''}</span>)}</div> : null}
          ...
          <div style={panel}><div style={panelBody}>No race record is currently available for this year.</div></div>
        )}
      </section>
    </main>
  )
