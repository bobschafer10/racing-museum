...
function getIssueData(slug: string): IssuePageData | null {
  ...
  Array.isArray(meta.pages) && meta.pages.length > 0
    ? meta.pages.map((page: { label: string; image: string }) => ({
        label: page.label,
        image: `/media/newspapers/${slug}/${page.image}`,
      }))
    : imageFiles.map((file, index) => {
        ...
        return {
          label,
          image: `/media/newspapers/${slug}/${file}`,
        }
      })
    ...
    return {
      ...
      issueTitle: meta.issueTitle || meta.issueDate || slug,
      issueDate: meta.issueDate || meta.issueTitle || slug,
      volume: meta.volume || "",
      number: meta.number || "",
      summary:
        meta.summary ||
        "Historic racing newspaper issue featuring race coverage, results, photos, standings, and regional reporting.",
      coverImage: `/media/newspapers/${slug}/${coverFile}`,
      pages,
      ...
      <section style={heroWrap}>
        <div style={heroGrid}>
          ...
          <div style={heroText}>
            ...
            <div style={publicationLine}>{issue.issueDate}</div>
            ...
            <p style={summaryText}>{issue.summary}</p>
                  ...
                  <div style={buttonRow}>
        <Link href="/media/newspapers" style={primaryButton}>
          Back to Newspapers
        </Link>
      </div>
    </div>
