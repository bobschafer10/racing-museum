...
const TARGET = "http://www.racingonline.com/cedarlake/stand.html";

export const dynamic = "force-dynamic";

export async function GET(req: NextRequest) {
  const key = req.nextUrl.searchParams.get("key");
  if (key !== "umarm-cedarlake-1997-2000") {
    return NextResponse.json({ error: "Not found" }, { status: 404 });
  }

  const ts = req.nextUrl.searchParams.get("ts");
  const original = req.nextUrl.searchParams.get("original") || TARGET;
  ...
  try {
    if (ts) {
      ...
      if (!/^https?:\/\/(www\.)?racingonline\.com(?::80)?\/cedarlake\//i.test(original)) {
        return NextResponse.json({ error: "Invalid original URL" }, { status: 400 });
      }
      const snapshotUrl = `https://web.archive.org/web/${ts}id_/${original}`;
    ...
    cdx.searchParams.set("from", "1997");
